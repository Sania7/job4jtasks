...
public class SetIterator {
    /**
     * 0.2. Итератор для Set.
     * Важной особенностью интерфейса Set является то,
     * что в нем не определен метод,
     * который позволяет напрямую получить значение из множества
     * (по аналогии с List, у Set нет метода get()).
     * Что же делать? Ведь нам же как-то надо получать значения.
     * Для этого можно использовать Iterator,
     * который работает по тому же принципу как у List
     ...
     *
     * E next() - метод перемещает курсор на следующий элемент в наборе и
     * при этом возвращает его.
     *
     * Задание: вам необходимо раскомментировать код,
     * заполнить коллекцию строками от one до five.
