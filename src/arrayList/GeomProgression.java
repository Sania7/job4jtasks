...
/**
 * 1.6. Геометрическая прогрессия
 * Геометри́ческая прогрессия — последовательность чисел
 * (называемых членами прогрессии),
 * в которой каждое последующее число, начиная со второго,
 * получается из предыдущего умножением его на определённое число q
 * (называемое знаменателем прогрессии).
 * Т.е. любой член геометрической прогрессии можно высчитать по формуле:
 *
 * b(n) = b(1) * q ^ (n - 1) - где значения в скобках означат номер
 ...
 *
 * 3. Количество элементов, которое должно содержаться в сгенерированной
 * последовательности.
 *
...
public class GeomProgression {
    public static int generateAndSum(int first, int denominator, int count) {
        ...
        for (int i = 0; i < count; i++) {
            int number = (int) (first * Math.pow(denominator, i));
            sum += number;
        }
        return sum;
    }
